...
def create_prompt(sample: dict, tokenizer: AutoTokenizer) -> dict:
    ...
    system_msg = {
        ...
        "content": (
            ...
            f"All items must be strictly within room dimensions (length x width, where x corresponds to length and y to width): {room_dims_str}, with a minimal gap of ~0.3m, and NO overlaps between objects. "
...
class JSONGeneratorAgent2D:
    def __init__(self, tokenizer, model: PreTrainedModel):
    ...
    def generate_layout(self, prompt: str) -> str:
        ...
        inputs = self.tokenizer(prompt, return_tensors="pt", truncation=True, max_length=MAX_LENGTH).to(accelerator.device)
    ...
    def calculate_batch_reward(self, completions: List[str], prompts: List[str], **kwargs) -> torch.Tensor:
        ...
        return torch.tensor(rewards, dtype=torch.float32, device=accelerator.device).clone().detach()
...
class TrainingMonitor(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if not accelerator.is_main_process or not logs:
            return
...
class ModelOutputLogger(TrainerCallback):
    ...
    def __init__(self, agent: JSONGeneratorAgent2D):
        self.agent = agent
...
def main():
    ...
    agent = JSONGeneratorAgent2D(tokenizer, model)
